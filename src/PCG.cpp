...
// Drawing - Mouse Input to paint the environment
void PCG::TileMap::Update() {
    Vector2 mousePos = GetMousePosition();
    int centerTileX = (int)(mousePos.x / PCG::TILE_SIZE);
    int centerTileY = (int)(mousePos.y / PCG::TILE_SIZE);

    if (IsMouseButtonDown(MOUSE_BUTTON_LEFT)) {
        for (int y = -brushSize; y <= brushSize; y++) {
            for (int x = -brushSize; x <= brushSize; x++) {
                if (x * x + y * y <= brushSize * brushSize) {
                    SetTile(centerTileX + x, centerTileY + y, activeBrushType);
                }
            }
        }
    }
...
char PCG::TileMap::GetTileChar(PCG::TileType _tileType) const {
    ...
    case PCG::TileType::TILE_TYPE_WATER:
        return PCG::WATER_CHAR;
    case PCG::TileType::TILE_TYPE_SAND:
        return PCG::SAND_CHAR;

    case PCG::TileType::TILE_TYPE_DIRT:
        return PCG::DIRT_CHAR;
...
void PCG::TileMap::LoadMapData(const char* _filename) {
    ...
    for (int y = 0; y < PCG::MAP_ROWS; y++) {
        for (int x = 0; x < PCG::MAP_COLUMNS; x++) {
            ...
            else if (ch == PCG::DIRT_CHAR) { 
                tileArray[y][x] = PCG::TileType::TILE_TYPE_DIRT;
            }
        }
    }
...
void PCG::TileMap::DrawGUI() {
    ...
    // Grass Button
    Rectangle grassRect = { PCG::BUTTON_X - 550, PCG::BUTTON_Y - 35, PCG::BUTTON_WIDTH -100, PCG::BUTTON_HEIGHT };
    if (GuiButton(grassRect, "Grass Tile")) {
        activeBrushType = TILE_TYPE_GRASS;
    }

    // Rock Button
    Rectangle rockRect = { PCG::BUTTON_X - 450, PCG::BUTTON_Y - 35, PCG::BUTTON_WIDTH - 100, PCG::BUTTON_HEIGHT };
    if (GuiButton(rockRect, "Rock Tile")) {
        activeBrushType = TILE_TYPE_ROCK;
    }

    // Water Button
    Rectangle waterRect = { PCG::BUTTON_X - 350, PCG::BUTTON_Y - 35, PCG::BUTTON_WIDTH - 100, PCG::BUTTON_HEIGHT };
    if (GuiButton(waterRect, "Water Tile")) {
        activeBrushType = TILE_TYPE_WATER;
    }

    // Sand Button
    Rectangle sandRect = { PCG::BUTTON_X - 250, PCG::BUTTON_Y - 35, PCG::BUTTON_WIDTH - 100, PCG::BUTTON_HEIGHT };
    if (GuiButton(sandRect, "Sand Tile")) {
        activeBrushType = TILE_TYPE_SAND;
    }

    Rectangle dirtRect = { PCG::BUTTON_X - 150, PCG::BUTTON_Y - 35, PCG::BUTTON_WIDTH - 100, PCG::BUTTON_HEIGHT };
    if (GuiButton(dirtRect, "Dirt Tile")) {
        activeBrushType = TILE_TYPE_DIRT; 
    }

    // Brush Size Slider
    Rectangle brushRect = { PCG::BUTTON_X - 775, PCG::BUTTON_Y - 35, PCG::BUTTON_WIDTH, PCG::BUTTON_HEIGHT };

    float tempBrush = (float)brushSize;
    GuiSliderBar(brushRect, "-", "+", &tempBrush, 0, 7);
    brushSize = (int)tempBrush;


    auto* noiseGen = dynamic_cast<PCG::NoiseMapGenerator*>(mapGenerator);
    if (noiseGen != nullptr) {
        float NoiseGuiX = PCG::BUTTON_X - 730;
        float NoiseGuiY = PCG::BUTTON_Y - 230;

        float NoiseGuiHeight = PCG::BUTTON_HEIGHT - 30;

        float spacing = 40.0f;

        // Slider for Noise Scale
        GuiSliderBar({ NoiseGuiX, NoiseGuiY, PCG::BUTTON_WIDTH, NoiseGuiHeight }, "Noise Scale", TextFormat("%2.1f", noiseGen->scale), &noiseGen->scale, 1.0f, 10.0f);

        // Sliders for Terrain Thresholds
        GuiSliderBar({ NoiseGuiX, NoiseGuiY + spacing, PCG::BUTTON_WIDTH, NoiseGuiHeight }, "Water Level", TextFormat("%2.2f", noiseGen->waterThreshold), &noiseGen->waterThreshold, 0.0f, 1.0f);
        GuiSliderBar({ NoiseGuiX, NoiseGuiY + spacing * 2, PCG::BUTTON_WIDTH, NoiseGuiHeight }, "Sand Level", TextFormat("%2.2f", noiseGen->sandThreshold), &noiseGen->sandThreshold, 0.0f, 1.0f);
        GuiSliderBar({ NoiseGuiX, NoiseGuiY + spacing * 3, PCG::BUTTON_WIDTH, NoiseGuiHeight }, "Grass Level", TextFormat("%2.2f", noiseGen->grassThreshold), &noiseGen->grassThreshold, 0.0f, 1.0f);
    }
}
...
void PCG::NoiseMapGenerator::Generate(TileType _tileArray[MAP_ROWS][MAP_COLUMNS]) {
    ...
    for (int y = 0; y < MAP_ROWS; y++) {
        for (int x = 0; x < MAP_COLUMNS; x++) {
            ...
            float brightness = col.r / 255.0f;

            // Tile colours are assigned based on "elevation"
            if (brightness < waterThreshold) {
                _tileArray[y][x] = TILE_TYPE_WATER;
            }
            else if (brightness < sandThreshold) {
                _tileArray[y][x] = TILE_TYPE_SAND;
            }
            else if (brightness < grassThreshold) {
